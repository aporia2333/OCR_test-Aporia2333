...
with st.sidebar:
    ...
    if mode in ["大模型", "视觉识别 API + 大模型"]:
        ...
        llm_provider = st.selectbox(
            "LLM 提供方", options=provider_options, index=default_provider_index, key="llm_provider_select"
        )
    ...
    output_format = st.selectbox("输出格式", options=["text", "json", "markdown"], index=0, key="output_format_select")
    language_hint = st.selectbox(
        "语言提示", options=["自动", "中文", "英文", "中英混合"], index=0, key="language_hint_select"
    )
